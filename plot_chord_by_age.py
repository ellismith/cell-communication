#!/usr/bin/env python3
"""
plot_chord_by_age.py
--------------------
Create chord diagrams for each age group with CONSISTENT nodes.
...
out_dir = os.path.join(base_dir, "figures/chord_by_age")
...
# Filter to within-age-group and significant
same_age = data[data["source_age"] == data["target_age"]].copy()
sig = same_age[same_age["magnitude_rank"] < 0.05].copy()
sig["age_group"] = sig["source_age"]

print(f"Significant same-age interactions: {len(sig):,}")

# Get ALL cell types across all age groups
...
# Create one chord diagram per age group
for age_group in age_groups:
    print(f"{'='*70}")
    print(f"Creating chord diagram for {age_group}")
    print(f"{'='*70}")
    
    age_data = sig[sig["age_group"] == age_group].copy()
    
    # Aggregate: mean lr_means per cell type pair
    flow = (age_data.groupby(["source_celltype", "target_celltype"])
           .agg({
               "lr_means": "mean",
               "interaction": "nunique"
           })
    ...
    # Add all cell types as nodes (even if no edges)
    ...
    # Create circular layout with FIXED positions for consistency
    ...
    ax.set_title(f"{age_group.capitalize()} Age Group\nCell Type Communication Network",
    ...
    outfile = os.path.join(out_dir, f"chord_age_{age_group}.png")
    plt.savefig(outfile, dpi=300, bbox_inches='tight', facecolor='white')
    print(f"  ✓ Saved {outfile}\n")
    plt.close()

print(f"{'='*70}")
print(f"✓ All age chord diagrams saved to {out_dir}")
print(f"{'='*70}")
