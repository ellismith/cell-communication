#!/usr/bin/env python3
"""
Generate pairwise CellChat jobs for EACH INDIVIDUAL animal
Each animal gets its own separate analysis with preflight checks
Preserves animal_id, age, sex, and louvain metadata
"""
import itertools
import os
import scanpy as sc
...
job_dir = "u01_per_individual_jobs"
...
if not os.path.exists(expected_counts_path):
    ...
    print("Please run preflight_check_u01.py first!")
    exit(1)
...
for individual in all_individuals:
    job_name = f"individual_{individual}"
...
print("="*70)
print("Analyzing Individual: {individual}")
print("="*70)
...
# Add individual metadata - INCLUDING LOUVAIN
...
# Check if louvain exists and add it
if 'louvain' in adata_combined.obs.columns:
    # For each source/target in results, we need to map back to the cells
    # But CellChat results are aggregated by cell_type_region, not individual cells
    # So we'll save the louvain info separately as a summary
    ...
    louvain_path = os.path.join(output_dir, f"{individual}_louvain_summary.csv")
...
print(f"\n✓ Created {len(all_individuals)} job scripts (one per individual)")
print(f"\nTo submit all jobs:")
