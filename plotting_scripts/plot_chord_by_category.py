...
#!/usr/bin/env python3
"""
Chord plots for significant interactions in a given functional category.
Grid mode shows all 11 regions in 3col x 4row layout.

Usage:
    python plot_chord_by_category.py --category "Synaptic adhesion"
    python plot_chord_by_category.py --category "Glutamate signaling" --grid
    python plot_chord_by_category.py --category "Glutamate signaling" --grid --colorbar 1
    python plot_chord_by_category.py --list_categories
    python plot_chord_by_category.py --category "Synaptic adhesion" --region dlPFC
"""
...
parser.add_argument('--category', default='Synaptic adhesion')
...
def get_class(s):
    for k in ['Glutamatergic','Astrocyte','Microglia','Ependymal','Vascular',
              'Cerebellar','Basket','Midbrain','GABA','Oligo','OPC','MSN']:
        if s.startswith(k):
            return k
    return 'Unknown'
...
if len(cat_pairs) == 0:
    print("No LR pairs found. Use --list_categories to see options.")
    exit(1)
...
for region in REGIONS:
    csv = BASE_DIR / "regression_results" / f"regression_{region}" / f"whole_{region.lower()}_age_sex_regression.csv"
    if not csv.exists():
        print(f"Missing: {csv}")
        continue
    ...
    print(f"  {region}: {len(sig)} sig interactions, {len(edges)} edges")
...
def draw_on_ax(ax, region, edges, use_norm, show_cbar=False):
    ...
    for ct in celltypes:
        ...
        ax.text(lx, ly, label, ha=ha, va=va, fontsize=14, fontweight='bold',
                color=get_color(ct),
                path_effects=[pe.withStroke(linewidth=2, foreground='white')])

    ax.set_xlim(-1.6, 1.6); ax.set_ylim(-1.6, 1.6)
    ax.set_aspect('equal'); ax.axis('off')
    ax.set_title(region, fontsize=20, fontweight='bold', pad=4)
    ...
    if show_cbar:
        ...
        cbar.set_label('Age Effect', fontsize=14)
        cbar.ax.tick_params(labelsize=8)

# ── SINGLE REGION ──────────────────────────────────────────────────────────
if args.region:
    if args.region not in all_edges:
        print(f"No significant interactions for {args.region}")
        exit(0)
    edges = all_edges[args.region]
    coef_abs_max = max(edges['mean_coef'].abs().max(), 0.001)
    use_norm = TwoSlopeNorm(vmin=-coef_abs_max, vcenter=0, vmax=coef_abs_max)
    fig, ax = plt.subplots(figsize=(9, 9))
    draw_on_ax(ax, args.region, edges, use_norm, show_cbar=False)
    sm = ScalarMappable(cmap=cmap, norm=use_norm); sm.set_array([])
    plt.colorbar(sm, ax=ax, shrink=0.35, pad=0.02).set_label('Age Effect', fontsize=18)
    ax.set_title(f"{args.region} — {args.category}\n", # Color=mean age effect | Width=n sig LR pairs (q<0.05)",
                 fontsize=20, fontweight='bold', pad=20)
    plt.tight_layout()
    fname = OUT_DIR / f"{args.region.lower()}_chord_{cat_slug}.png"
...
# ── GRID MODE ──────────────────────────────────────────────────────────────
    ...
    fname = OUT_DIR / f"grid_chord_{cat_slug}{suffix}.png"
    fig.savefig(fname, dpi=200, bbox_inches='tight')
    plt.close()
    print(f"✓ {fname.name}")
