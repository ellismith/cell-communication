...
Usage:
  ...
  python plot_hypergeometric_heatmaps.py --log log2 --clip --sig_only --split_colors
"""
...
def make_heatmap(df, direction, role, log_fn, log_label, out_dir, log_name,
                 clip, clip_tag, sig_only, sig_tag, split_colors, col_tag):
    if direction == "strengthening":
        fe_col, q_enrich, q_deplete = "fold_enrichment_pos","q_pos_enrich","q_pos_deplete"
    else:
        fe_col, q_enrich, q_deplete = "fold_enrichment_neg","q_neg_enrich","q_neg_deplete"
    ...
    plt.tight_layout()
    fname = f"heatmap_{direction}_{role}_{log_name}{clip_tag}{sig_tag}{col_tag}.png"
...
for direction in ["strengthening", "weakening"]:
    for role in ["sender", "receiver"]:
        make_heatmap(df, direction, role, log_fn, log_label,
                     OUT_DIR, args.log, args.clip, clip_tag,
                     args.sig_only, sig_tag, args.split_colors, col_tag)
