#!/usr/bin/env python3
"""
Run LIANA CellChat analysis for the first individual using the latest CellPhoneDB interaction CSV only.
Compatible with LIANA 1.4.0
"""
...
# ------------------- SETTINGS -------------------
individual = "0B9"   # First individual
...
interaction_csv = "/scratch/easmit31/cell_cell/CellPhoneDB_latest/interaction_input_CellPhoneDB.csv"
...
# ------------------- LOAD INTERACTIONS -------------------
print("Loading latest CellPhoneDB interaction CSV...")
interaction_df = pd.read_csv(interaction_csv)
print(f"Loaded {len(interaction_df):,} interactions")

# Convert to list of tuples: (partner_a, partner_b)
interactions_list = list(zip(interaction_df['partner_a'], interaction_df['partner_b']))

print(f"Converted to {len(interactions_list)} interaction tuples")
print(f"Example interactions: {interactions_list[:5]}")
