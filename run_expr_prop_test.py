#!/usr/bin/env python3
"""
Generate CellChat jobs for HIP/ACC regions, Glutamatergic/GABA only
...
for expr_prop in expr_prop_thresholds:
    ...
    print(f"\n=== Creating jobs for expr_prop = {expr_prop} ===")
...
'''
    ...
    print(f"✓ Created {len(all_individuals)} job scripts for expr_prop={expr_prop}")

print(f"\n\n{'='*80}")
print(f"SUMMARY")
print(f"{'='*80}")
print(f"Created jobs for {len(expr_prop_thresholds)} expr_prop thresholds")
print(f"Thresholds: {expr_prop_thresholds}")
print(f"Cell types: Glutamatergic, GABA")
print(f"Regions: HIP, ACC")
print(f"Individuals: {len(all_individuals)}")
print(f"\nTo submit all jobs for a specific threshold:")
print(f"  for f in {base_job_dir}/jobs_filtering<X>p<Y>/*.sh; do sbatch $f; done")
print(f"\nExample:")
print(f"  for f in {base_job_dir}/jobs_filtering0p1/*.sh; do sbatch $f; done")
