#!/usr/bin/env python3
"""
plot_cross_region_sex_chords.py
--------------------------------
Create chord diagrams showing cross-region communication for each sex.
...
out_dir = os.path.join(base_dir, "figures/cross_region_sex_chords")
...
# Create chord for each sex
for sex in sex_groups:
    print(f"{'='*70}")
    sex_label = "Female" if sex == "F" else "Male"
    print(f"Creating cross-region chord for {sex_label}")
    print(f"{'='*70}")
    
    sex_data = sig[sig["sex_group"] == sex].copy()
    
    # Aggregate: mean lr_means per region pair
    flow = (sex_data.groupby(["source_region", "target_region"])["lr_means"]
           .mean()
           .reset_index())
    ...
    outfile = os.path.join(out_dir, f"cross_region_chord_{sex}.png")
    plt.savefig(outfile, dpi=300, bbox_inches='tight', facecolor='white')
    print(f"  ✓ Saved {outfile}\n")
    plt.close()

print(f"{'='*70}")
print(f"✓ All cross-region sex chords saved to {out_dir}")
print(f"{'='*70}")
