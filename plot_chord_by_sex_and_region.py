#!/usr/bin/env python3
"""
plot_chord_by_sex_and_region.py
--------------------------------
Create chord diagrams for each sex within each region.
Shows how cell type communication varies by sex within specific brain regions.
"""
import os, glob
import pandas as pd
...
out_dir = os.path.join(base_dir, "figures/chord_by_sex_and_region")
...
# Filter to within-region, within-sex, and significant
within = data[(data["source_sex"] == data["target_sex"]) & 
...
print(f"Significant within-region, within-sex interactions: {len(sig):,}")
...
# Get sex groups and regions
sex_groups = sorted(sig["sex_group"].unique())
regions = sorted(sig["region"].unique())
print(f"Sex groups: {sex_groups}")
print(f"Regions: {regions}\n")

# Create chord diagram for each sex × region combination
for sex_group in sex_groups:
    for region in regions:
        print(f"{'='*70}")
        print(f"Creating chord: {region} - {sex_group}")
        print(f"{'='*70}")
        
        subset = sig[(sig["sex_group"] == sex_group) & (sig["region"] == region)].copy()
        ...
        ax.set_title(f"{region} Region - {sex_label}\nCell Type Communication",
        ...
        outfile = os.path.join(out_dir, f"chord_{region}_{sex_group}.png")
        plt.savefig(outfile, dpi=300, bbox_inches='tight', facecolor='white')
        print(f"  ✓ Saved {outfile}\n")
        plt.close()

print(f"{'='*70}")
print(f"✓ All sex×region chord diagrams saved to {out_dir}")
print(f"{'='*70}")
